...
cat >"$TEST_ROOT/codex-memory/tasks.json" <<'EOF'
{
  "tasks": [
    {
      "id": "task-snapshot-running",
      "title": "Reduce duplicate dashboard reads",
...
state=running
project=registry-smoke
task=Reduce duplicate dashboard reads
last_result=NONE
note=lane=1
updated_at=2026-03-24T00:01:00Z
EOF
...
printf '{"project":"registry-smoke","task":"Reduce duplicate dashboard reads","result":"SUCCESS","attempts":1,"score":1}\n' >"$TEST_ROOT/codex-memory/tasks.log"
printf "Reduce duplicate dashboard reads\n" >"$TEST_ROOT/queues/registry-smoke.txt"
...
TRACKED_FILES=(
  ...
  "$TEST_ROOT/codex-memory/dashboard-settings.json"
  "$TEST_ROOT/codex-learning/external-signals.json"
  "$TEST_ROOT/codex-logs/strategy-latest.json"
  "$TEST_ROOT/status.txt"
  "$TEST_ROOT/codex-memory/tasks.log"
  "$TEST_ROOT/queues/registry-smoke.txt"
  "$TEST_ROOT/codex-logs/codex-auth-failure.json"
)
...
DASHBOARD_AUTH_FAILURE_FILE="$TEST_ROOT/codex-logs/codex-auth-failure.json" \
...
assert payload["queue"]["tasks"][0]["task"] == "Reduce duplicate dashboard reads"
PY
...
echo "dashboard snapshot read dedupe test passed"
