...
if [ "$resolved_status" != "shelved" ]; then
  echo "expected resolved failed structured seed to be shelved, got: $resolved_status" >&2
  exit 1
fi
...
echo "strategy auto approve external resolved failed structured seed shelve test passed"
