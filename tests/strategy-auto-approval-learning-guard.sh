...
(
  cd "$TEST_ROOT"
  bash agents/strategy.sh codex-agent-system "$TMP_DIR/strategy-auto-learning-guard.json" >/dev/null
)

python3 - "$TEST_ROOT" "$TMP_DIR/strategy-auto-learning-guard.json" <<'PY'
import json
import os
import sys
...
assert output["data"]["board_tasks"] == [
    ...
    {
        "id": "task-002-make-active-worker-ownership-and-progres",
        "action": "created",
        "source_task_id": "enterprise-readiness",
    },
]
...
assert created["task-002-make-active-worker-ownership-and-progres"]["status"] == "pending_approval"
assert created["task-002-make-active-worker-ownership-and-progres"]["strategy_template"] == "enterprise_live_work_observability"
...
echo "strategy auto approval learning guard test passed"
