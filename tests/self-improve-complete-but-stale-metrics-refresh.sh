...
touch -t 202603250650 "$REPO_ROOT/codex-learning/metrics.json"
touch -t 202603250700 "$REPO_ROOT/codex-memory/tasks.json"
touch -t 202603250700 "$REPO_ROOT/codex-memory/tasks.log"
touch -t 202603250700 "$REPO_ROOT/codex-learning/external-signals.json"
...
if [ "${self_improve_count:-0}" -ne 0 ]; then
  echo "expected stale complete metrics to refresh before self-improve task generation" >&2
  exit 1
fi
...
if [ "$artifact_summary" != $'0\t1.0\t0.0\tno_detected_weakness' ]; then
  echo "unexpected self-improve artifact after stale complete metrics refresh: $artifact_summary" >&2
  exit 1
fi
...
if [ "$metrics_summary" != $'1.0\t0.0\t0\t1' ]; then
  echo "expected stale complete metrics refresh to overwrite prior values: $metrics_summary" >&2
  exit 1
fi
...
if [ "$metrics_input_summary" != $'refreshed\ttrue\tstale_against_tasks_json\t' ]; then
  echo "unexpected stale metrics input summary: $metrics_input_summary" >&2
  exit 1
fi

echo "self improve complete-but-stale metrics refresh test passed"
