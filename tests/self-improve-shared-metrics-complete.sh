...
touch -t 209901010512 "$TEST_ROOT/codex-learning/metrics.json"
touch -t 209901010510 "$TEST_ROOT/codex-memory/tasks.json"
touch -t 209901010511 "$EXTERNAL_PROJECT_ROOT/.codex-agent/tasks.json"
...
if [ "$summary" != $'complete\tfalse\tcomplete_snapshot' ]; then
  echo "unexpected self-improve shared-metrics summary: $summary" >&2
  exit 1
fi

echo "self improve shared metrics complete test passed"
