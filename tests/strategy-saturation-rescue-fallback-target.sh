    ...
    [
        {
            ...
            "failed_at": "2026-03-23T20:05:00Z"
        },
        {
            ...
            "failed_at": "2026-03-23T20:15:00Z"
        }
    ]
)
...
(
  cd "$TEST_ROOT"
  bash agents/strategy.sh codex-agent-system "$TMP_DIR/strategy-saturation-rescue-fallback-target.json" >/dev/null
)

python3 - "$TEST_ROOT" "$TMP_DIR/strategy-saturation-rescue-fallback-target.json" <<'PY'
import json
import os
import sys
...
assert output["data"]["board_updates"] == [
    {
        "id": "task-001-replace-cut-queue-timeout-churn-before-r",
        "action": "created",
        "source_task_id": "strategy::saturation-recovery",
    }
]
...
created = next(
    ...
    if task.get("id") == "task-001-replace-cut-queue-timeout-churn-before-r"
)

assert created["status"] == "pending_approval"
assert created["title"] == "Replace Cut queue timeout churn before retries burn worker capacity with a different bounded experiment"
...
echo "strategy saturation rescue fallback target test passed"
