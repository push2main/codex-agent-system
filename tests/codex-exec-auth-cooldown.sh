...
set +e
PATH="$FAKE_BIN_DIR:$PATH" CODEX_CALL_COUNT_FILE="$CALL_COUNT_FILE" CODEX_AUTH_FAILURE_COOLDOWN_SECONDS=3600 run_codex_exec test-role "$PROJECT_DIR" "test prompt" "$OUTPUT_FILE"
first_rc=$?
PATH="$FAKE_BIN_DIR:$PATH" CODEX_CALL_COUNT_FILE="$CALL_COUNT_FILE" CODEX_AUTH_FAILURE_COOLDOWN_SECONDS=3600 run_codex_exec test-role "$PROJECT_DIR" "test prompt" "$OUTPUT_FILE"
second_rc=$?
set -e
