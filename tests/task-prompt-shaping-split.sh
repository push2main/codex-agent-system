    ...
    data=json.dumps(
        {
            ...
            "prompt": "Tighten the dashboard header spacing, then verify the dashboard screenshots.",
        }
    ).encode("utf-8"),
...
assert [task["title"] for task in payload["tasks"]] == [
    "Tighten the dashboard header spacing",
    "Verify the dashboard screenshots",
]
PY

echo "task prompt shaping split test passed"
