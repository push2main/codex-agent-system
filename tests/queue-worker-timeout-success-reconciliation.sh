...
mkdir -p \
  ...
  "$TEST_ROOT/projects/timeout-success" \
  "$TEST_ROOT/queues"
...
command = (
    ...
    "\"timeout-success\" "
    "\"timeout success task\" "
    "\"SUCCESS\" "
    "\"run-timeout-success-001\" "
    "\"1\" "
    "\"2\" "
    "\"100\" "
    "\"299\" "
...
cat >"$TEST_ROOT/codex-memory/tasks.json" <<'EOF'
{
  "tasks": [
    {
      "id": "task-timeout-success",
      "title": "timeout success task",
      "project": "timeout-success",
      "status": "approved",
...
assert execution_context["result"] == "SUCCESS"
assert execution_context["step_count"] == 2
assert execution_context["completed_steps"] == 2
...
records = [line for line in records_path.read_text().splitlines() if line.strip()]
assert records == []
PY

echo "queue worker timeout success reconciliation test passed"
