...
if ! printf '%s\n' "$pending_summary" | grep -Fq $'Add canonical incident example for credential recovery required\tpackages/schema/incident.schema.json'; then
  echo "expected canonical incident example task for missing external schema coverage, got: $pending_summary" >&2
  exit 1
fi

if ! printf '%s\n' "$pending_summary" | grep -Fq 'incident_type: "credential_recovery_required"'; then
  echo "expected schema-gap task to ground the missing example on incident_type, got: $pending_summary" >&2
  exit 1
fi

if printf '%s\n' "$pending_summary" | grep -Fq 'Break retry churn'; then
  echo "expected external control-plane retry churn task to be suppressed, got: $pending_summary" >&2
  exit 1
fi

echo "self improve external product gap generation test passed"
