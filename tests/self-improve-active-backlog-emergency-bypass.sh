...
if [ "$task_summary" != $'4\tReduce timeout rate' ]; then
  echo "expected zero-step timeout emergency to bypass the active self-improve backlog cap: $task_summary" >&2
  exit 1
fi
...
if [ "$artifact_summary" != $'2\t1\tcritical_low_success_rate\t2\t3' ]; then
  echo "unexpected emergency bypass artifact summary: $artifact_summary" >&2
  exit 1
fi

echo "self improve active backlog emergency bypass test passed"
