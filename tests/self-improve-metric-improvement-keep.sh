...
cat >"$REPO_ROOT/codex-learning/metrics.json" <<'EOF'
{
  ...
  "retry_classification_coverage": 0.6,
...
if [ "$task_summary" != $'completed\tkept\t\t0.6\t60% (6/10)\texecute_success' ]; then
  echo "unexpected kept self-improve task summary: $task_summary" >&2
  exit 1
fi
...
if [ "$artifact_summary" != $'0\t0\t1\tnone' ]; then
  echo "unexpected self-improve artifact kept summary: $artifact_summary" >&2
  exit 1
fi

echo "self improve metric-improvement keep test passed"
