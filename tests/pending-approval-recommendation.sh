  ...
  "tasks": [
    {
      ...
      "title": "Check the oldest pending approval",
    ...
    },
    {
      ...
      "title": "Check the newer pending approval",
      "project": "codex-agent-system",
...
for _ in range(30):
    ...
    raise SystemExit("dashboard recommendation endpoint did not become ready")

summary = registry["summary"]
assert summary["topPendingTask"]["id"] == "task-pending-older"
assert summary["approvalRecommendation"]["task_id"] == "task-pending-older"
assert summary["approvalRecommendation"]["pending_approval_count"] == 2
assert summary["nextAction"]["state"] == "approval"
assert summary["nextAction"]["message"] == (
    "Review oldest pending task first: Check the oldest pending approval (2 pending approvals)."
)

with urllib.request.urlopen(f"{base_url}/api/metrics", timeout=1) as response:
    metrics = json.load(response)

assert metrics["approvalRecommendation"]["task_id"] == "task-pending-older"
assert metrics["approvalRecommendation"]["pending_approval_count"] == 2
assert metrics["nextAction"]["message"] == (
    "Review oldest pending task first: Check the oldest pending approval (2 pending approvals)."
)
PY

echo "pending approval recommendation test passed"
