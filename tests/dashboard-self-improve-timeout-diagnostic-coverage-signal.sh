...
cat >"$TEST_ROOT/codex-learning/self-improve-run.json" <<'EOF'
{
  "status": "success",
  "project": "codex-agent-system",
  "generated_at": "2026-03-25T16:45:00Z",
  "counts": {
    "detected": 1,
    "generated": 1,
    "submitted": 1,
  ...
  "gating": {
    ...
    "overload": {
      "active": true,
      "preserved_title": "Improve timeout diagnostic coverage",
...
for _ in range(30):
    try:
        with urllib.request.urlopen(f"{base_url}/api/project-summaries", timeout=1) as response:
            project_summaries = json.load(response)
...
summary = project_summaries["projects"][0]["self_improve_summary"]
overview = project_summaries["projects"][0]["project_overview_signal"]
remediation = summary["operator_signal"]["remediation"]
dashboard_signal = dashboard["metrics"]["selfImprove"]["operator_signal"]

expected_summary = "21% diagnostic coverage (91/443 failures) while 91% of timeout failures ended before any step started; self-improve prioritized Improve timeout diagnostic coverage."
expected_detail = "Improve deterministic failed-step timeout evidence before retrying generic timeout-reduction work."

assert summary["operator_signal"]["kind"] == "timeout_diagnostic_coverage_low", summary
assert summary["operator_signal"]["title"] == "Low timeout diagnostic coverage", summary
assert summary["operator_signal"]["summary"] == expected_summary, summary
assert remediation["active"] is True, remediation
assert remediation["kind"] == "repair_timeout_diagnostics", remediation
assert remediation["title"] == "Repair timeout diagnostics first", remediation
...
assert overview["kind"] == "timeout_diagnostic_coverage_low", overview
assert overview["title"] == "Low timeout diagnostic coverage", overview
assert overview["summary"] == expected_summary, overview
assert overview["detail"] == expected_detail, overview

assert dashboard_signal["kind"] == "timeout_diagnostic_coverage_low", dashboard_signal
assert dashboard_signal["summary"] == expected_summary, dashboard_signal
PY

echo "dashboard self-improve timeout diagnostic coverage signal test passed"
