...
- 2026-03-25T07:50:00Z | weakness=Improve retry success rate | improvement=Improve retry success rate | outcome=success submitted=1/2 detected=2 duration_s=1 | next=Reduce timeout rate | external_sync_pending=false
EOF
...
if [ "$submitted_title" != "Reduce timeout rate" ]; then
  echo "expected automation memory next hint to prioritize timeout reduction, got: $submitted_title" >&2
  exit 1
fi

echo "self improve automation memory next preference test passed"
