...
cat >"$REPO_ROOT/codex-learning/external-signals.json" <<'EOF'
{
  ...
  "signals": [
    {
      "source_id": "superheld-release-feed",
      "source_label": "Superheld release feed",
      "title": "Superheld release 1.2.3",
      "url": "https://example.invalid/superheld/releases/1.2.3",
      "published_at": "2026-03-29T20:45:00Z",
      "fetched_at": "2026-03-29T21:00:00Z"
    }
  ],
  "errors": []
}
...
if [ "${pending_count:-0}" != "2" ]; then
  echo "expected two grounded pending_approval tasks, got: $pending_count" >&2
  exit 1
fi
...
timeout_summary="$(
  jq -r '
    .tasks[]
    | select(.status == "pending_approval" and .title == "Reduce timeout rate")
    | [
        (.target_files // [] | join(",")),
...
if ! printf '%s\n' "$timeout_summary" | grep -F $'scripts/verify-baseline.sh\tscripts/verify-baseline.sh\tStart with `scripts/verify-baseline.sh` at the top-level verification block immediately after `require_pattern()`.' >/dev/null; then
  echo "expected timeout task to ground onto scripts/verify-baseline.sh, got: $timeout_summary" >&2
  exit 1
fi
...
retry_summary="$(
  jq -r '
    .tasks[]
    | select(.status == "pending_approval" and .title == "Improve retry success rate")
    | [
        (.target_files // [] | join(",")),
        (.task_intent.affected_files // [] | join(",")),
        (.reason // "")
...
if ! printf '%s\n' "$retry_summary" | grep -F $'scripts/lib.sh\tscripts/lib.sh\tStart with `scripts/lib.sh` at insert a retry-failure context helper immediately after `baseline_verification_fail()` and before `run_baseline_verification()`.' >/dev/null; then
  echo "expected retry task to ground onto scripts/lib.sh, got: $retry_summary" >&2
  exit 1
fi

echo "self improve external history grounding test passed"
