...
tasks.append(
    {
        "id": "task-old-rejected-saturation-rescue",
...
(
  cd "$TEST_ROOT"
  bash agents/strategy.sh codex-agent-system "$TMP_DIR/strategy-saturation-rescue-rejected-retarget.json" >/dev/null
)

python3 - "$TEST_ROOT" "$TMP_DIR/strategy-saturation-rescue-rejected-retarget.json" <<'PY'
import json
import os
import sys
...
echo "strategy saturation rescue rejected retarget test passed"
