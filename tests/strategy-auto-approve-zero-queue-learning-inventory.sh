...
generic_status="$(
  jq -r '.tasks[] | select(.id == "task-generic-self-improve") | .status' "$TEST_ROOT/workspaces/superheld/.codex-agent/tasks.json"
)"
inventory_status="$(
...
if [ "$generic_status" != "pending_approval" ]; then
  echo "expected generic self-improve task to remain pending in zero-queue fast path, got: $generic_status" >&2
  exit 1
fi

if [ "$inventory_status" != "approved" ]; then
  echo "expected bounded learning inventory task to auto-approve after 90s with zero queue, got: $inventory_status" >&2
  exit 1
fi
...
echo "strategy auto approve zero queue learning inventory test passed"
