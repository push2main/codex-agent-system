...
(
  ...
  SELF_IMPROVE_TITLE_FAMILY_FAILURE_STREAK_THRESHOLD=2 \
...
if [ "${task_count:-0}" -ne 0 ]; then
  echo "expected saturated timeout family to stay blocked after repeated failures" >&2
  exit 1
fi

echo "self improve title family saturation test passed"
