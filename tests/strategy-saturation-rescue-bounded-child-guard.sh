...
tasks.append(
    {
        ...
        "execution_context": {
            "failed_step": "Run `bash scripts/run-playwright-docker.sh bash tests/dashboard-screenshot-verification.sh` as the single deterministic verification command before retrying the broader rescue.",
            "step_count": 5
        },
...
bounded_children = [
    ...
    and task.get("source_task_id") == "strategy::saturation-recovery"
]
assert bounded_children == []
...
created = next(
    ...
    if task.get("id") != "task-failed-saturation-rescue"
)
assert created["status"] == "pending_approval"
assert created["title"] == "Replace Cut dashboard task-registry read amplification before growth stalls the loop with a different bounded experiment"
...
echo "strategy saturation rescue bounded-child guard test passed"
