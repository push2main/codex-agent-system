...
cat >"$TEST_ROOT/codex-memory/tasks.json" <<'EOF'
{
  "tasks": [
    {
      "id": "task-timeout-reconciled",
      "title": "Reconcile late success after timeout",
      "project": "codex-agent-system",
      "category": "stability",
      "status": "completed",
      "created_at": "2026-03-24T07:58:00Z",
      "updated_at": "2026-03-24T08:06:00Z",
      "completed_at": "2026-03-24T08:06:00Z",
      "execution": {
        "state": "completed",
        "attempt": 2,
        "max_retries": 2,
...
task=
last_result=SUCCESS
note=timeout_reconciled=1
updated_at=2026-03-24T08:06:00Z
EOF

cat >"$TEST_ROOT/codex-memory/tasks.log" <<'EOF'
{"timestamp":"2026-03-24T08:00:00Z","project":"codex-agent-system","task":"Reconcile late success after timeout","result":"FAILURE","failure_kind":"timeout","task_id":"task-timeout-reconciled","attempts":2,"score":0,"run_id":"run-timeout-reconciled"}
EOF
...
assert payload["timeoutFailure"] == 0, payload
assert payload["timeoutFailureRate"] == 0, payload
PY

echo "dashboard api timeout metrics alignment test passed"
