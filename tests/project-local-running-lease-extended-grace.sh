...
print((datetime.now(timezone.utc) - timedelta(seconds=20)).strftime("%Y-%m-%dT%H:%M:%SZ"))
PY
)"
...
cat >"$EXTERNAL_PROJECT_ROOT/.codex-agent/tasks.json" <<EOF
{
  "tasks": [
    {
      "id": "task-superheld-extended-grace",
      "title": "project local extended grace task",
      "project": "superheld",
      "status": "running",
...
if [ -s "$TMP_DIR/reconciled.txt" ]; then
  echo "expected no reconciliation inside extended grace window" >&2
  cat "$TMP_DIR/reconciled.txt" >&2
  exit 1
fi

if [ -s "$TEST_ROOT/queues/superheld.txt" ]; then
  echo "expected no queue rehydrate during extended grace window" >&2
  exit 1
fi
...
echo "project local running lease extended grace test passed"
