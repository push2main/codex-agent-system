...
export function buildIncidentSummary(event, incidentType) {
  if (incidentType === "social_message_risk") {
    return "Suspicious message or fake profile requires review.";
  }
  if (incidentType === "phishing_link_risk") {
    return "Suspicious link may lead to credential theft.";
  }
  if (incidentType === "media_authenticity_risk") {
    return "Reported media may be manipulated and needs safe review.";
  }
  throw new Error(`unsupported incident type: ${incidentType}`);
}
EOF
...
if ! printf '%s\n' "$pending_summary" | grep -Fq $'Add credential recovery support to incident flow\tapps/cloud-brain/src/incident-flow.mjs'; then
  echo "expected incident-flow credential recovery task, got: $pending_summary" >&2
  exit 1
fi

if ! printf '%s\n' "$pending_summary" | grep -Fq 'INCIDENT_TYPE_BY_EVENT'; then
  echo "expected incident-flow task to ground the edit on INCIDENT_TYPE_BY_EVENT, got: $pending_summary" >&2
  exit 1
fi

if ! printf '%s\n' "$pending_summary" | grep -Fq 'LEARNING_MODULES_BY_INCIDENT'; then
  echo "expected incident-flow task to ground the edit on LEARNING_MODULES_BY_INCIDENT, got: $pending_summary" >&2
  exit 1
fi

echo "self improve external incident flow gap generation test passed"
