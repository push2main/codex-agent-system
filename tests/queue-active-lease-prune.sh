...
mkdir -p \
  ...
  "$TEST_ROOT/projects/lease-prune-smoke" \
  "$TEST_ROOT/queues"
...
cat >"$TEST_ROOT/codex-memory/tasks.json" <<'EOF'
{
  "tasks": [
    {
      "id":"task-running",
      "title":"active leased task",
      "project":"lease-prune-smoke",
      "status":"running",
      ...
      "execution":{
        ...
        "lane":"lane-9",
        "lease_state":"claimed",
        "lease_expires_at":"2099-03-23T08:10:00Z"
      },
      "history":[]
    },
    {
...
cat >"$TEST_ROOT/queues/lease-prune-smoke.txt" <<'EOF'
active leased task
next approved task
EOF
...
queue_path = root / "queues" / "lease-prune-smoke.txt"
...
for _ in range(40):
    ...
    if approved.get("status") == "completed" and "active leased task" not in remaining:
        break
    time.sleep(0.25)
else:
    raise SystemExit("expected duplicate active-lease task to be pruned and next task to complete")
PY
...
echo "queue active lease prune test passed"
