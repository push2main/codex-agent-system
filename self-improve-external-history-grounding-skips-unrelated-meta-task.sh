...
cat >"$EXTERNAL_WORKSPACE/.codex-agent/tasks.json" <<'EOF'
{
  "tasks": [
    ...
    },
    {
      ...
      "updated_at": "2026-03-29T20:21:47Z"
    }
  ]
}
EOF
...
if jq -e '.tasks[] | select(.status == "pending_approval" and .title == "Refresh stale external signals")' \
  "$EXTERNAL_WORKSPACE/.codex-agent/tasks.json" >/dev/null; then
  echo "expected stale external signal task to be skipped for unrelated external-project grounding" >&2
  exit 1
fi
...
if ! printf '%s\n' "$timeout_summary" | grep -F $'scripts/verify-baseline.sh\tStart with `scripts/verify-baseline.sh` at the top-level verification block immediately after `require_pattern()`.' >/dev/null; then
  echo "expected timeout task to remain grounded onto scripts/verify-baseline.sh, got: $timeout_summary" >&2
  exit 1
fi

echo "self improve external history grounding skips unrelated meta task test passed"
